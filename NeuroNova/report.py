...
risk level, medicine adherence and reminders, so a family member or ASHA
worker can hand it to a doctor / PHC without needing the app open.
...
GAME_LABELS = {"memory_match": "Memory Match", "pattern_recall": "Pattern Recall", "family_match": "Who Is This?"}
...
def generate_caregiver_report(patient, days=21):
    ...
    story.append(Paragraph(
        ...
        f"For caregiver / ASHA worker / physician use",
        sub_style,
    ))
    ...
    patient_table = Table(
        [
            ...
            ["Village", patient["village"], "Language", language_name],
        ],
    ...
    story.append(Paragraph(
        "This report is generated by Neuronova's trend-detection engine (a transparent linear-regression "
        "model over logged game scores) for caregiver and ASHA-worker reference only. It is not a clinical "
        "diagnosis — please consult a physician or the nearest primary health centre for medical evaluation.",
        disclaimer_style,
    ))
